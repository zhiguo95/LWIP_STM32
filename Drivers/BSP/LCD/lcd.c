...
void lcd_init(void)
{
    ...
    /* FSMC读时序控制寄存器 */
    fsmc_read_handle.AddressSetupTime = 0x0F;           /* 地址建立时间(ADDSET)为16个fmc_ker_ck 1/168M=6ns*16=96ns */
    ...
    fsmc_write_handle.AddressSetupTime = 9;             /* 地址建立时间(ADDSET)为9个fmc_ker_ck=54ns */
    fsmc_write_handle.AddressHoldTime = 0x00;
    fsmc_write_handle.DataSetupTime = 9;                /* 数据保存时间(DATAST)为9个fmc_ker_ck=54ns */
